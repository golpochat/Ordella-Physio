...
import {
  ...
  installProvisioningFailFromQuery,
  listTenants,
  loginSuperAdmin,
  ...
  waitForGateway,
} from "../infrastructure/developer-tooling-layer/playwright/helpers/provisioning-rollback-e2e";

test.describe("Super-admin provisioning rollback (gateway + Docker dev stack)", () => {
  test.beforeAll(async () => {
    test.skip(
      !provisioningE2eReady(),
      "API gateway URL is required for provisioning rollback E2E",
    );
  ...
  test("1. successful provisioning", async ({ page }) => {
  ...
  test("2. failure at org creation → no tenant, no owner, no Stripe customer", async ({
    page,
  }) => {
    const superAdmin = await loginSuperAdmin();
    const markers = buildProvisioningMarkers(uniqueProvisioningSuffix(), "tenant-level");
    const removeInjection = installProvisioningFailFromQuery(page);

    try {
      await fillProvisioningWizard(page, markers, { failAt: "org" });
      await submitProvisioningWizard(page);
  ...
  test("3. failure at tenant creation → rollback org + Stripe", async ({ page }) => {
    const superAdmin = await loginSuperAdmin();
    const markers = buildProvisioningMarkers(uniqueProvisioningSuffix(), "tenant-level");
    const removeInjection = installProvisioningFailFromQuery(page);

    try {
      await fillProvisioningWizard(page, markers, { failAt: "tenant" });
      await submitProvisioningWizard(page);
  ...
  test("4. failure at owner assignment → rollback org + tenant", async ({ page }) => {
    const superAdmin = await loginSuperAdmin();
    const markers = buildProvisioningMarkers(uniqueProvisioningSuffix(), "tenant-level");
    const removeInjection = installProvisioningFailFromQuery(page);

    try {
      await fillProvisioningWizard(page, markers, { failAt: "owner" });
      await submitProvisioningWizard(page);
  ...
  test("5. failure at billing setup → rollback all entities", async ({ page }) => {
    const superAdmin = await loginSuperAdmin();
    const markers = buildProvisioningMarkers(uniqueProvisioningSuffix(), "tenant-level");
    const removeInjection = installProvisioningFailFromQuery(page);

    try {
      await fillProvisioningWizard(page, markers, { failAt: "billing" });
      await submitProvisioningWizard(page);
  ...
  test("retry works after injected failure via ?failAt=tenant", async ({ page }) => {
    const superAdmin = await loginSuperAdmin();
    const suffix = uniqueProvisioningSuffix();
    const markers = buildProvisioningMarkers(suffix, "tenant-level");
    const removeInjection = installProvisioningFailFromQuery(page);
