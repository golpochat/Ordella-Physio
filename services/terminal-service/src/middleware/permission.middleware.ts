...
function sendForbidden(response: Response): void {
  response.status(403).json(terminalForbiddenError().serialize());
}

export function hasTerminalManageAccess(user: {
  role: SecurityRole;
  permissions?: string[];
}): boolean {
  if (user.role === "SYSTEM") {
  ...
  if (user.permissions?.includes(PERMISSIONS.TERMINAL_MANAGE)) {
    return true;
  }
...
export function requirePermission(_permission: typeof PERMISSIONS.TERMINAL_MANAGE) {
  return (request: Request, response: Response, next: NextFunction): void => {
    ...
    if (!hasTerminalManageAccess(user)) {
      sendForbidden(response);
      return;
    }
