 ...
 * TODO: Replace with Elasticsearch/OpenSearch client (`ELASTICSEARCH_URL`, credentials).
 */
@Injectable()
export class ElasticsearchProvider implements SearchProviderAdapter {
  ...
  indexDocument(indexKey: string, document: SearchDocument): Promise<void> {
    this.logger.debug(`[elasticsearch-stub] index ${indexKey}/${document.id}`);
