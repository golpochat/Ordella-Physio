...
import type { ClinicNote } from "@/lib/clinic-portal-types";
import { formatPortalDate } from "@/lib/clinic-portal-utils";

export function ClinicNotesList({ notes }: { notes: ClinicNote[] }) {
  if (!notes.length) {
    return (
      <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">
        <p className="font-medium text-foreground">No clinical notes</p>
        <p className="mt-2">Therapist notes will appear here for oversight.</p>
      </div>
    );
  }
