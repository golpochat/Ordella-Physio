...
import { USER_PORTAL_NAV } from "@/lib/user-portal-nav";
import { useUiStore } from "@/store/ui.store";

export function UserPortalShell({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const { logout } = useAuth();
  const { user, displayName } = useUserPortalContext();
          ...
          >
            <div className={cn("border-b p-4", collapsed && "px-2")}>
              {!collapsed ? (
                ...
                <p className="text-center text-xs font-semibold">U</p>
              )}
            </div>
            <nav className="flex flex-1 flex-col gap-1 overflow-y-auto p-3">
              {USER_PORTAL_NAV.map((link) => {
                const active =
                  link.href === "/user"
                    ? pathname === "/user"
