...
type ClinicLocationDetailPageProps = {
  params: { id: string };
};

export default function ClinicLocationDetailPage({ params }: ClinicLocationDetailPageProps) {
  ...
  return (
    <WithPermission permission="location.manage">
      <PageHeader
        title={data?.name ?? "Location details"}
        subtitle="View location details and manage status."
        action={
      ...
      {!isLoading && !isError && data ? <LocationDetail location={data} /> : null}
    </WithPermission>
  );
}
