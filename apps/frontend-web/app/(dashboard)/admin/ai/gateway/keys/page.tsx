    ...
    <WithPermission permission="ai.admin">
      <AiAdminShell>
        <ListPage
          ...
          subtitle="Scoped API keys for inference, embeddings, training, and datasets."
          isLoading={isLoading}
          isError={isError}
        ...
        >
          ...
          {canManage ? (
            <ApiKeyEditor
              ...
              onSave={(payload) => {
                void createKey.mutateAsync(payload).then((result) => {
          ...
          <ApiKeyList
            ...
            onToggleActive={
              canManage
                ? (id, isActive) => void updateKey.mutateAsync({ id, payload: { isActive } }).then(() => toast.success("Updated."))
                : undefined
            }
            onRotate={
              canManage
                ? (id) => void rotateKey.mutateAsync(id).then((result) => {
                    setCreatedApiKey(result.apiKey);
                    toast.success("Key rotated.");
                  })
                : undefined
            }
            onRevoke={canManage ? (id) => void revokeKey.mutateAsync(id).then(() => toast.success("Key revoked.")) : undefined}
          />
        </ListPage>
      </AiAdminShell>
    </WithPermission>
